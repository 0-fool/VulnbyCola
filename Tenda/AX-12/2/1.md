...
In the sub_42E030 function, the list parameter is passed to the sscanf function and parsed into the v16 buffer. The v16 buffer has a size of 16 bytes (4 * _DWORD), but there is no restriction on the length of the list parameter. When the first field's length exceeds 15 bytes (requiring 1 byte for the null terminator), sscanf writes excessive data to v16, causing a stack overflow. This may lead to a Denial of Service (DoS) attack or potential code execution.
...
list=AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA,field2,field3,field4
```
